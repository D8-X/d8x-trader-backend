...
export class SetOracles {
	...
	public async insertSetOraclesRecord(
		...
		const S2 =
			this.fromBytes4(e.baseQuoteS2[0]) + "-" + this.fromBytes4(e.baseQuoteS2[1]);
		const S3 =
			this.fromBytes4(e.baseQuoteS3[0]) + "-" + this.fromBytes4(e.baseQuoteS3[1]);
		await this.insertPerpetualLongId(
			"SetOraclesEvent",
			perpId,
			S2,
			S3,
